...
또한, 수료 직후에는 동료의 창업 프로젝트에 합류하여 약 한 달간 하이브리드 캐주얼 게임을 리딩하며 Firebase 연동을 통한 로그인, 랭킹, 친구 기능을 구현했습니다. 데이터 설계 경험도 함께 쌓았고, 실무 환경에 적응하여 전반을 리딩한 소중한 경험이 되었습니다. 비록 정식 고용 구조 문제로 프로젝트는 종료되었지만, 스타트업 환경에서의 실전 경험은 큰 자산이 되었습니다.`
            },
            {
                ...
                content: `팀 프로젝트에서는 클라이언트 개발자로서 기술 구현뿐 아니라 팀원들과의 협업을 통해 문제를 함께 해결하는 데 주력해왔습니다.

VR FPS 프로젝트에서는 VR 기기 원점 초기화 문제로 플레이어 장비 위치가 어긋나는 버그가 발생했을 때, 문제를 직접 파악하고 팀원들과 해결 방안을 논의해 위치 동기화 시스템을 개선했습니다. 씬 전환 시 플레이어 상태가 초기화되는 이슈도 공유된 구조 설계를 통해 중앙 매니저와 상태 저장 시스템을 도입함으로써 팀 전체의 개발 효율을 높일 수 있었습니다.

또한, 게임 씬 추가 후 매니저 초기화 순서 오류로 NullReferenceException이 반복되던 상황에서는 팀원들과 함께 상태 관리 방식을 조정하고, 매니저 간 의존성 구조를 재정리하여 문제를 해결했습니다. 이 과정에서 서로의 코드를 이해하고 조율하는 협업 경험이 특히 인상 깊었습니다.

RPG 프로젝트에서는 몬스터 AI가 Terrain 지형에 제대로 반응하지 않는 문제에 대해 팀과 함께 원인을 분석하고 개선 방법을 공유하면서, 실제 게임 환경에 적합한 로직으로 함께 수정해나갔습니다.

이처럼 개발 중 발생하는 다양한 문제를 팀원들과 적극적으로 공유하고, 함께 해결하며 시스템 안정성과 팀워크를 동시에 강화한 경험은 실무 환경에서도 큰 강점이 될 것이라 자신합니다.`
            },
            {
                title: "5. 포부",
